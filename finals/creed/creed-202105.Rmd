...
.center[
  ![:scale 100%](../pics/kff-uninsured.png)
]
...
- Denote by $N_{j}^{1} = \sum_{i} 1(Y_{ij}^{1}=1 | \text{Assist})$ the the count of enrollees in plan $j$ had they received decision assistance, conditional on receiving assistance

- Denote by $N_{j}^{0} = \sum_{i} 1(Y_{ij}^{0}=1 | \text{Assist})$ the count of enrollees in plan $j$ had they not received decision assistance, conditional on receiving assistance
...
2. Use $\hat{\theta}$ to form predicted counterfactual, $\hat{N}_{j}^{0} = \sum_{i} \widehat{Pr[Y_{ij}^{0} | \text{Assist}]}$

3. Use predicted value in ATT estimate: $$\underbrace{N_{j}^{1}}_{Observed} - \underbrace{\hat{N}_{j}^{0}}_{Predicted}$$

4. Aggregate to level of insurer and metal tier
