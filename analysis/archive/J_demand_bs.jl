...
#	min_beta = newton_nested_logit(beta_initial,X,choices,1e-8)

##### Estimation with Other methods	
	obj_nested_logit(betap) = objective_nested_logit(betap,X,choices);

#	nested_logit = optimize(obj_nested_logit, g!, beta_initial, LBFGS()) ;
	nested_logit = try
			optimize(obj_nested_logit, beta_initial, LBFGS(); autodiff = :forward)
		catch
			optimize(obj_nested_logit, beta_initial, iterations=50000)
		end
