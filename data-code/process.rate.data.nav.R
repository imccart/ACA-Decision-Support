# Load Data
	...
	# MLR Data
	mlr_data <- read.csv("data/mlr_data.csv",header=TRUE)
...
# Create cost targets and firm level statistics (e.g., admin costs)	
	...
	write.csv(conv_output,"convergence_analysis_updated.csv")
...
#### Calculate risk scores
	...
	# Need:
		...
		write.csv(ra_output,"ra_by_metal_year3.csv")
...
##### Prepare Demand Data
	...
	# Prepare demand data
		...
		demand_data <- read.csv(file="ca_julia_data_FEB052021_small.csv",header=TRUE)
		...
		filename <- paste("choice_probs_2014_",year_to_run,".csv",sep="");
...
##### Run risk score regressions	
	...
	filename <- paste("rs_coefficients_nodem_",year_to_run,"_V11_learn.csv",sep="");
	write.csv(coef(risk_score_alt_learn),filename)
	write.csv(summary(risk_score_alt_learn)[5],paste("reg_output_V11_learn_",year_to_run,".csv",sep=""))
	if(year_to_run == 2018) save(risk_score_alt,file="rs_reg_V11_learn")
	...
	if(use_alt_version & !learning) {
		...
		filename <- paste("rs_coefficients_nodem_",year_to_run,".csv",sep="");
		write.csv(coef(risk_score_mktfe),filename)
		write.csv(summary(risk_score_mktfe)[5],"reg_output.csv")
		if(year_to_run == 2018) save(risk_score_mktfe,file="rs_reg")
	}
...
# Run Average Claims Regression on rsdata observations only
	...
	filename = paste("claims_initial_",year_to_run,".csv",sep="")
...
##### Save and output data
	...
	if(year == 2018) {
		...
		filename = paste("moments_data_V11_nav.csv",sep="")
